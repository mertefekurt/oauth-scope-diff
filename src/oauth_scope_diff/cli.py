...
from oauth_scope_diff import __version__
from oauth_scope_diff.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from oauth_scope_diff.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='oauth-scope-diff',
        description='Detect OAuth scope creep in app manifest changes.',
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help='OAuth manifest diff or scope inventory',
    )
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"oauth-scope-diff: error: {exc}", file=sys.stderr)
        return 1
